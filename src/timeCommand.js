...
class timeCommand {
  /**
   * Interpret and process a budbot time command
  ...
  static async process(msg, commandPrefix, command, args) {
    if (command !== "time") {
      msg.reply(`budbot error: Time command routing error \`(command=${command})\`. \
  Please report this to a Chingu administrator.`);
      console.log(
        `budbot error: Time command routing error (command=${command})`
      );
      return;
    }
    ...
    if (args[0] === "in") {
      ...
      const cityName = args
        .reduce((cityName, currentArg, currentIndex) => {
          if (currentIndex !== 0) {
      ...
      if (cities.length <= 0) {
        ...
        msg.reply(
          `In ${cities[0].city} it is currently ${date.toLocaleTimeString(
            "en-US",
            options
          )}`
        );
        ...
        cities.forEach(city => {
          ...
          msg.reply(
            `In ${city.city}, ${state} ${
              city.country
            }  it is currently ${date.toLocaleTimeString("en-US", options)}`
          );
        });
        return;
      }
