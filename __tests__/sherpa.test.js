...
describe("000.00/Test the Sherpa Discord bot", () => {
  ...
  test("100.01/Validate the ask command", async () => {
    ...
    expect(msg.getLastReply().indexOf("budbot error")).toBeGreaterThan(-1);

    askCommand.process(msg, commandPrefix, "ask", []);
    expect(msg.getLastReply()).toMatch(
      "https://chingu.docs.chingu.io/about/askhelp"
    );
  ...
  test("200.01/Validate the doc command", async () => {
    ...
    expect(msg.getLastReply().indexOf("I'm sorry")).toBeGreaterThan(-1);

    docCommand.process(msg, commandPrefix, "doc", []);
    expect(msg.getLastReply()).toMatch("https://docs.chingu.io");

    docCommand.process(msg, commandPrefix, "doc", ["chingu"]);
    expect(msg.getLastReply()).toMatch("https://chingu.docs.chingu.io");

    docCommand.process(msg, commandPrefix, "doc", ["pair"]);
    expect(msg.getLastReply()).toMatch("https://pairprog.docs.chingu.io");

    docCommand.process(msg, commandPrefix, "doc", ["proj"]);
    expect(msg.getLastReply()).toMatch("https://projres.docs.chingu.io");

    docCommand.process(msg, commandPrefix, "doc", ["tech"]);
    expect(msg.getLastReply()).toMatch("https://techres.docs.chingu.io");

    docCommand.process(msg, commandPrefix, "doc", ["voyage"]);
    expect(msg.getLastReply()).toMatch("https://voyage.docs.chingu.io");
  ...
  test("300.01/Validate the help command", async () => {
    ...
    expect(
      msg.getLastReply().indexOf(" __**Sherpa Commands:**__ ")
    ).toBeGreaterThan(-1);

    helpCommand.process(msg, commandPrefix, "help", ["ask"]);
    expect(
      msg.getLastReply().indexOf("**Sherpa ask Commands:**")
    ).toBeGreaterThan(-1);

    helpCommand.process(msg, commandPrefix, "help", ["doc"]);
    expect(
      msg.getLastReply().indexOf("**Sherpa doc Commands:**")
    ).toBeGreaterThan(-1);

    helpCommand.process(msg, commandPrefix, "help", ["issue"]);
    expect(
      msg.getLastReply().indexOf("**Sherpa issue Commands:**")
    ).toBeGreaterThan(-1);

    helpCommand.process(msg, commandPrefix, "help", ["sched"]);
    expect(
      msg.getLastReply().indexOf("**Sherpa schedule Commands:**")
    ).toBeGreaterThan(-1);
    ...
    expect(
      msg.getLastReply().indexOf("**Sherpa social Commands:**")
    ).toBeGreaterThan(-1);

    helpCommand.process(msg, commandPrefix, "help", ["time"]);
    expect(
      msg.getLastReply().indexOf("**Sherpa time Commands:**")
    ).toBeGreaterThan(-1);
  ...
  test("400.01/Validate the issue command", async () => {
    ...
    expect(msg.getLastReply()).toMatch(
      "https://chingu.docs.chingu.io/about/rptissue"
    );
  ...
  test("500.01/Validate the sched|schedule commands", async () => {
    ...
    expect(msg.getLastReply()).toMatch(
      "https://chingu.docs.chingu.io/intro/schedule"
    );
  ...
  test("700.01/Validate the time command", async () => {
    ...
    expect(msg.getLastReply().indexOf("In BadCityName")).toEqual(-1);

    timeCommand.process(msg, commandPrefix, "time", ["in", "Chicago"]);
    expect(msg.getLastReply().indexOf("In Chicago")).toBeGreaterThan(-1);

    timeCommand.process(msg, commandPrefix, "time", ["in", "Springfield"]);
    expect(msg.getLastReply().indexOf("In Springfield,")).toBeGreaterThan(-1);
